...
class PerPromptStatTrackerNFT:
    ...
    def update(self, prompts, rewards, dists=None, exp=False):
        ...
        for prompt in unique:
            ...
            # min_val = np.min(avg_distances)
            # max_val = np.max(avg_distances)
            prompt_dists = avg_distances#/np.mean(avg_distances)#(avg_distances - min_val) / (max_val - min_val)
            ...
            advantages[prompts == prompt] = (prompt_rewards - mean) * (prompt_dists)# / std
        return advantages,prompt_dists
    def get_stats(self):
